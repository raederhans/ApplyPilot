...
- Never submit when the exact LinkedIn job ID or canonical listing URL is already present in Applied."""
    ...
    lever_form_trick = (
        "- Lever: after the resume upload settles, take one fresh snapshot and keep its current "
        "refs for independent ordinary controls. Bulk-fill visible ordinary text fields and "
        "issue all visible native-select operations before taking one shared verification "
        "snapshot. Do not re-snapshot between successful independent native selects, and do not "
        "replace a native select with click or keyboard navigation while its current ref still "
        "works. This grouped rule does not apply to custom/autocomplete controls, multi-selects, "
        "conditional fields, validation failures, or a page rerender; handle those individually "
        "with fresh refs. Retry only the failed operation, never the whole group."
