#!/usr/bin/env python3
"""Install Job Apply Pilot as an isolated command-line application.
...
def resolve_source(root: Path) -> str:
    ...
    if len(wheels) > 1:
        raise RuntimeError("Release bundle contains more than one Job Apply Pilot wheel")
    if wheels:
        wheel = wheels[0]
...
def install(
    ...
    *,
    ...
    if sys.version_info < MIN_PYTHON:
        ...
        raise RuntimeError(f"Job Apply Pilot requires Python {required} or newer")
