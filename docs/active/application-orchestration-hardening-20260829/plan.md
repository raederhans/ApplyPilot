...
- [ ] Stage 8: Restore a hermetic green CI baseline on Python 3.11, 3.12, and 3.13 without requiring Codex CLI or optional CloakBrowser installation during unit tests. The three-file patch passes an isolated `905`-test baseline and portability probes; remote matrix confirmation remains.
...
1. **P0 hermetic baseline (implemented locally; remote gate pending)** — isolate CLI discovery and optional CloakBrowser from unit-test ambient state; assert the resume route command contract without dependency-version-specific help rendering. Proof: exact three regressions, adjacent contracts, full local suite, then remote Python 3.11/3.12/3.13 CI.
