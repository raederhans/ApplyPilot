...
## 2026-09-02 Happy Pilot P0-P4 execution wave

The user-approved architecture audit is implemented as five ordered promotion gates on the isolated branch `agent/happy-pilot-p0-p4-20260902`, based on `fork/main` commit `577c6f7de339ba1be4dfe7f18bd4c94fea3c7f22`. The dirty parent worktree at `C:\Users\raede\Desktop\简历\applypilot-local\source` is evidence-only and must not be edited, staged, reset, cleaned, or used as the integration target.

- [x] **P0 baseline and measurement:** prove run/session/profile/output namespaces cannot cross; shorten the global submit critical section without weakening reservation or receipt semantics; add process-spawn, MCP-ready, first-tool, RSS, token, and submit-lane metrics; change only user-visible branding from ApplyPilot to Happy Pilot while retaining package and CLI compatibility. Accepted locally on 2026-09-02 with the isolated `664 passed` P0 composition, scoped Ruff, and `git diff --check`.
- [x] **P1 persistent application sessions (functional slice):** bounded browser/runtime ownership, `ApplicationSupervisor`, compact `ContextBundle`, generation-bound restart policy, and a default-off loopback Streamable HTTP MCP seam are implemented. The isolated composition passed `1688` tests, scoped Ruff, and `git diff --check`. Real `@playwright/mcp@0.0.79` HTTP/CDP/Codex handshake and matched 1/2-worker throughput remain unverified, so the seam stays off and the production worker cap is unchanged; those are P4 admission evidence rather than a reason to block the frozen P2 interface.
- [x] **P2 semantic browser gateway (functional slice):** page-versioned `FormSurface`/`ControlDescriptor` inspection, HMAC single-use semantic capabilities, P1 root-target binding, and postcondition-checked Workday/SmartRecruiters routine operations are implemented. Unknown/ambiguous/stale controls, closed shadow roots, inaccessible cross-origin frames, coverage overflow, and Submit/file-path operations fail closed. The isolated suite passed `1699` tests and the final root-target regression passed `12` tests. The ordinary `run_job` path is not globally cut over to this gateway, so this is a frozen capability seam rather than a production rollout.
- [x] **P3 durable autonomy and recovery:** `ApplicationEpisode`, typed command/result events in the existing `agent_events` authority, source-backed `JobEvidenceBundle`, crash-safe effect-unknown parking, and one bounded same-application descriptor diff replan are implemented. A real non-submit worker seam consumes the contract. Missing/conflicting material facts still route to human handling, while safe park/handoff/receipt-check commands remain available. The corrected isolated suite passed `1708` tests.
- [x] **P4 evidence-driven concurrency mechanism:** a real local 1/2/4 no-submit cohort, fail-closed admission manifest, read-only operator surface, and clamp/downshift-only `PerformanceGovernor` are implemented and tested. The final 24-task, 2-warmup, 10-measured-block run is locally `NOT_QUALIFIED` and production `NOT_ADMITTED`: 2-worker speedup lower bound was `1.307x < 1.6x`, 4-worker was `1.367x < 2.6x`, and both p95 ratios exceeded `1.2x`. All safety, quality, RSS, SQLite, submit-lane, and zero-side-effect gates passed. The production cap/config remains unchanged.

Write authority is serial: P0-P3 functional slices are accepted locally, and P4 is now the sole code writer. `/root` owns this record, integration, long tests, browser/provider resources, commits, pushes, and final promotion decisions.

## 2026-09-03 integration audit corrections

The integration audit keeps the P0-P4 measurements and isolated mechanisms, but narrows production authority before merge:

- Public display naming is **CapyPilot**. Existing technical identifiers remain `applypilot`, `applypilot-local`, and `APPLYPILOT_*`; the temporary Happy Pilot name is not a compatibility contract.
- `ApplicationEpisode` and bounded descriptor replanning remain diagnostic/P4-only projections. Production recovery continues to execute solely through the existing durable `recovery_execution.execute_recovery_command` lifecycle; the duplicate worker wrapper was removed.
- Semantic control inspection/descriptors remain available, while the in-process HMAC control executor is reachable only from the isolated no-submit P4 worker and focused tests. Production `SemanticBrowserOps` exposes no generic `apply_control` method or control issuer/driver.
- Windows recycle telemetry uses current `WorkingSetSize`, not historical `PeakWorkingSetSize`, so a recovered process is not restarted forever after one transient peak.
